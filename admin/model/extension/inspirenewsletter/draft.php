<?php
class ModelExtensionInspirenewsletterDraft extends Model {

	// inspireo 2.2 edited
	public function getTotalEmail() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "inspirenewsletter_subscribe");
		return $query->row['total'];
	}

	public function getEmails() {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "inspirenewsletter_subscribe");
		return $query->rows;
	}

	public function deletedraft($draft_id){
		$this->db->query("DELETE FROM " . DB_PREFIX . "inspirenewsletter_draft WHERE draft_id = '" . (int)$draft_id . "'");
	}

	public function getTotalDraft() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "inspirenewsletter_draft");
		return $query->row['total'];
	}

	public function getDraft($draft_id){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "inspirenewsletter_draft WHERE draft_id = " . (int)$draft_id);
		return $query->row;
	}

	public function getDrafts($data = array()) {

		$sql = "SELECT * FROM " . DB_PREFIX . "inspirenewsletter_draft";
	...
	public function savebasic($data){
		...
		if (isset($data['draft_id']) && $data['draft_id'] != 0) {
			$this->db->query("UPDATE " . DB_PREFIX . "inspirenewsletter_draft SET 
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "inspirenewsletter_draft SET 
				`date_added` = NOW(),
				`to` = '" . $this->db->escape($data['to']) . "',
				`store_id` = '" . (int)$data['store_id'] . "',
				`subject` = '" . $this->db->escape($data['subject']) . "',
	...
	// inspireo 2.2 end edited
	
	public function getTotal($data = array()) {
		$sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "inspirenewsletter_email` WHERE 1=1";
	...
	public function getList($data = array()) {
		if ($data) {
			$sql = "SELECT `email_id`, `subject`, `date_added`, `to`, `store_id` FROM " . DB_PREFIX . "inspirenewsletter_email WHERE 1=1"; 
	...
	public function detail($email_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "inspirenewsletter_email WHERE email_id = '" . (int)$email_id . "'"); 
		$data = $query->row;

		if ($data) {
			$path = dirname(DIR_DOWNLOAD) . DIRECTORY_SEPARATOR . 'attachments' . DIRECTORY_SEPARATOR . 'inspiredraft_' . $data['email_id'];
	...
	public function delete($email_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "inspirenewsletter_email WHERE email_id = '" . (int)$email_id . "'");

		$path = dirname(DIR_DOWNLOAD) . DIRECTORY_SEPARATOR . 'attachments' . DIRECTORY_SEPARATOR . 'inspiredraft_' . $email_id;
		if (file_exists($path) && is_dir($path)) {
			$this->rrmdir($path);
		}
	}
