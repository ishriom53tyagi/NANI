...
class ModelExtensionInspirenewsletterNewsletter extends Model {
  ...
  public function createTableDraft(){
    $sql_table = "CREATE TABLE IF NOT EXISTS `".DB_PREFIX."inspirenewsletter_draft` (
    ...
    ";
    $sql_show = " SHOW TABLES LIKE '".DB_PREFIX."inspirenewsletter_draft'";
    $query = $this->db->query( $sql_show );
    if(count($query->rows)<=0){
      $query = $this->db->query( $sql_table );
    }
  }
  // inspireo 2.2 end edit 

	protected function createTables(){
		$sql = array();

		$sql[] = "CREATE TABLE IF NOT EXISTS `".DB_PREFIX."inspirenewsletter_email` (
		...
		";
		$sql[] = "CREATE TABLE IF NOT EXISTS `".DB_PREFIX."inspirenewsletter_subscribe` (
		...
		";
		$sql[] = "CREATE TABLE IF NOT EXISTS `".DB_PREFIX."inspirenewsletter_template` (
                ...
                ";
		$sql[] = "CREATE TABLE IF NOT EXISTS `".DB_PREFIX."inspirenewsletter_template_description` (
                  `template_id` int(11) DEFAULT NULL,
                  `language_id` int(11) DEFAULT NULL,
                  `subject` varchar(200) DEFAULT NULL,
		...
		$sql[] = "CREATE TABLE `".DB_PREFIX."inspirenewsletter_history` (
	...
	public function getBody($data) {
		...
		$template = $this->model_inspirenewsletter_template->getTemplate($data['template_id']);
    ...
    $data['message'] = str_replace( "{messages}", "", $data['message']);
		...
		$this->load->model("inspirenewsletter/product");
		if(isset($lstproducts['selected'])) {
			$setting['thead'] = $this->language->get('text_send_email_selected');
			$html_selected = $this->model_inspirenewsletter_product->genListProducts($lstproducts['selected'], $setting);
		...
		if(isset($lstproducts['special'])) {
			...
			$html_special = $this->model_inspirenewsletter_product->genListProducts($lstproducts['special'], $setting);
		...
		if(isset($lstproducts['latest'])) {
			...
			$html_latest = $this->model_inspirenewsletter_product->genListProducts($lstproducts['latest'], $setting);  
		...
		if(isset($lstproducts['popular'])) {
			...
			$html_popular = $this->model_inspirenewsletter_product->genListProducts($lstproducts['popular'], $setting);
		...
		if(isset($lstproducts['category'])) {
			...
			$html_category = $this->model_inspirenewsletter_product->genListProducts($lstproducts['category'], $setting);
	...
	public function send($data) {
        require_once(DIR_SYSTEM . 'library/mail_inspire.php');
    ...
    public function getCustomer($customer_id) {
        $query = $this->db->query("SELECT DISTINCT c.* FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "inspirenewsletter_subscribe ps ON c.email = ps.email WHERE c.customer_id = '" . (int)$customer_id . "'");

        return $query->row;
    }

    public function getAffiliates($data = array()) {
        $sql = "SELECT *, CONCAT(a.firstname, ' ', a.lastname) AS name, (SELECT SUM(at.amount) FROM " . DB_PREFIX . "affiliate_transaction at WHERE at.affiliate_id = a.affiliate_id GROUP BY at.affiliate_id) AS balance FROM " . DB_PREFIX . "affiliate a LEFT JOIN " . DB_PREFIX . "inspirenewsletter_subscribe ps ON a.email = ps.email";
    ...
    public function getAffiliate($affiliate_id) {
        $query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "affiliate a LEFT JOIN " . DB_PREFIX . "inspirenewsletter_subscribe ps ON a.email = ps.email WHERE affiliate_id = '" . (int)$affiliate_id . "'");
