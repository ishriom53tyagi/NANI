...
class ModelExtensionInspirenewsletterSubscribe extends Model { 
	...
	public function buildQuery($data = array(), $get_total = false){		
		if($get_total){
			$sql = "SELECT count(s.subscribe_id) AS total
				FROM ".DB_PREFIX."inspirenewsletter_subscribe AS s
	...
	public function updateAction($subscribe_id, $action = 1){
		$query = $this->db->query("UPDATE ".DB_PREFIX."inspirenewsletter_subscribe SET `action`=".(int)$action." WHERE subscribe_id=".$subscribe_id);
		return true;
	}

	public function delete($subscribe_id){
		$this->db->query("DELETE FROM `" . DB_PREFIX . "inspirenewsletter_subscribe` WHERE subscribe_id = '" . (int)$subscribe_id . "'");
	}
