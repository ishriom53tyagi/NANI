<?php
class ModelExtensionInspirenewsletterTemplate extends Model { 
	
	public function getTemplates($data = array()){
		$sql = "SELECT * FROM ".DB_PREFIX."inspirenewsletter_template LIMIT 0,20";
	...
	public function getTemplate($template_id){
		...
		if(!empty($template_id)){
			$sql = "SELECT t.* FROM ".DB_PREFIX."inspirenewsletter_template AS t 
					WHERE t.template_id = ".$template_id;
			$query =  $this->db->query($sql);
			if($query->num_rows > 0){
				$data = $query->row;
				$sql2 = "SELECT td.* FROM ".DB_PREFIX."inspirenewsletter_template_description AS td WHERE td.template_id = ".$template_id;
	...
	public function insertTemplate($data = array()){
		if(!empty($data["template"]["template_id"])){
			$sql = "UPDATE ".DB_PREFIX."inspirenewsletter_template SET ";
			$tmp = array();
			...
			$sql = "INSERT INTO ".DB_PREFIX."inspirenewsletter_template ( `";
		...
		if(!empty($template_id)){
			$sql = "DELETE FROM ".DB_PREFIX."inspirenewsletter_template_description WHERE `template_id`=".$template_id;
			$this->db->query($sql);
			$sql = "INSERT INTO ".DB_PREFIX."inspirenewsletter_template_description ( `template_id`,`language_id`,`subject`,`template_message` ) VALUES ";
	...
	public function delete($templates = array()){
		...
		if(!empty($templates)){
			...
			foreach($templates as $template){
				$sql = "DELETE FROM ".DB_PREFIX."inspirenewsletter_template WHERE template_id = ".(int)$template;
				$sql2 = "DELETE FROM ".DB_PREFIX."inspirenewsletter_template_description WHERE template_id=".(int)$template;
		...
		if(!empty($templates)){
			...
			foreach($templates as $template){
				...
				if($query->num_rows){
					...
					$sql = "SELECT td.language_id,td.subject,td.template_message FROM ".DB_PREFIX."inspirenewsletter_template_description AS td
