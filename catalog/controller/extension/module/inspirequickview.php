...
/**
 * @class ControllerExtensionModuleInspirelayerednavigation
 * @version 1.0
 */
		...
		switch( $route ){
			...
			case "":
				...
				$info = $this->model_setting_setting->getSetting( 'inspirequickview' );
		  		if( isset($info['inspirequickview']['status']) && $info['inspirequickview']['status'] ){
		  			$this->document->addStyle('catalog/view/javascript/jquery/inspirequickview.css' );
		  		}
			break;
		}
		return ;
	...
	public function show() {
		...
		if ($product_info) {
			...
			if ((float)$product_info['discount']) {
				$data['discount'] = $this->currency->format($this->tax->calculate($product_info['discount'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
			} else {
				$data['discount'] = false;
			}
